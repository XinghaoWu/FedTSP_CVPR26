...
def train_alignment(P_init, Q_init, method, num_epochs=100, lr=0.01):
    """
    Train alignment of P and Q using specified method
    ...
    for epoch in range(num_epochs):
        ...
        if method == 'cosine':
            loss = 1 - cosine_similarity(P, Q, detach_norm=False)
        elif method == 'cosine_detach':
            loss = 1 - cosine_similarity(P, Q, detach_norm=True)
        elif method == 'cka':
            loss = 1 - linear_cka(P, Q, detach_norm=False)
        elif method == 'cka_detach':
            loss = 1 - linear_cka(P, Q, detach_norm=True)
        elif method == 'mse':
            loss = mse_loss(P, Q)
        else:
...
def main():
    ...
    # Initialize P and Q with different norms to observe the effect of alignment methods
    # P: smaller norm, Q: larger norm
    P_init = torch.randn(k, D) * 0.5  # Scale down P
    Q_init = torch.randn(k, D) * 2.0  # Scale up Q
    ...
    # Define five methods
    methods = [
        ('cosine', '1-cosine alignment'),
        ('cosine_detach', '1-cosine alignment (detach denominator)'),
        ('cka', '1-CKA alignment'),
        ('cka_detach', '1-CKA alignment (detach denominator)'),
        ('mse', 'MSE alignment')
    ]
    ...
    for method, method_name in methods:
        ...
        # Train
        P_final, Q_final = train_alignment(
            P_init, Q_init,
            method=method,
