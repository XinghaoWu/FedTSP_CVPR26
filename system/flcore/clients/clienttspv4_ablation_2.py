...
1. The local alignment loss can be changed to L2 loss.
'''

class clientTSPv4_ablation_2(Client):
    ...
    def train(self):
        ...
        for step in range(max_local_epochs):
            for i, (x, y) in enumerate(self.trainloader):
                ...
                if self.args.local_loss_type == 'Con':
                    logit_scale = self.logit_scale.exp()
                    logits_per_image = logit_scale * image_features @ text_features.t()
                    loss2 = F.cross_entropy(logits_per_image, y)
                elif self.args.local_loss_type == 'L2':
                    # 取每个样本对应类别的text_features
                    # print('Using L2 loss')
                    target_text_features = text_features[y]
                    loss2 = self.loss_mse(image_features, target_text_features)
                else:
                    raise ValueError(f"Unknown local_loss_type: {self.args.local_loss_type}")
