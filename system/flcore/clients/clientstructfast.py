            ...
            for i, (x, y) in enumerate(trainloader):
                ...
                if self.args.version == 1:
                    if global_protos is not None:
                        ...
                        valid_mask = torch.tensor([type(global_protos[yy.item()]) != type([]) for yy in y], device=self.device)
                        valid_indices = torch.where(valid_mask)[0]
                        valid_protos = torch.stack([global_protos[y[vi].item()].data for vi in valid_indices])
                        proto_new[valid_indices] = valid_protos
                ...
                if accumulate_features:
                    for cls in torch.unique(y):
                        cls_mask = (y == cls)
                        protos[cls.item()].append(rep[cls_mask].detach())
...
def agg_func(protos):
    ...
    for label, proto_list in protos.items():
        if len(proto_list) > 1:
            # 优化：使用 torch.cat 和 mean 替代手动求和
            proto_list = [p.view(-1, p.size(-1)) for p in proto_list]
            proto_stack = torch.cat(proto_list, dim=0)
            protos[label] = proto_stack.mean(dim=0)
        else:
