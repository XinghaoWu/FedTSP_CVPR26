import time
import numpy as np
from flcore.clients.clienttspv4_ablation_2 import clientTSPv4_ablation_2
...
相比于FedTSPv4_ablation, FedTSPv4_ablation_2主要区别在于：
1. 训练trainable prompt时可以添加FedTGP loss
2. 本地训练loss可以改为L2 loss
'''


class FedTSPv4_ablation_2(Server):
    def __init__(self, args, times):
        ...
        if 'main.py' in self.caller_script:
            ...
            logger_path = (
                ...
                f'ep{args.local_epochs}_bs{args.batch_size}_nc{args.num_clients}/lr({args.local_learning_rate}_{args.prompt_lr})_'
                f'lamda(t{args.lamda})_cls{args.p_classifier}_prompt(CSC{args.CSC}_len{args.len_prompt}_random{args.prompt_random_init}_manual{args.manual_prompt}_negative{args.negative_class})_'
                f'EMA{args.EMA_alpha}_promptep{args.prompt_epoch}_{args.server_model}_LLM({args.LLM_prompt_file}_{args.LLM_prompt_number})_TGP({args.TGP_loss})_local_loss({args.local_loss_type})_test({args.visualization_mode}_{args.visualization_dataset_type}_{args.test_data_mode})_seed{args.seed}/')
        ...
        if 'main.py' in self.caller_script:
            ...
            self.final_log_path = f'../visualization_logs/{args.dataset}/{args.model_family}/{args.algorithm}/gr{args.global_rounds}_ep{args.local_epochs}_bs{args.batch_size}_nc{args.num_clients}/test({args.visualization_mode}_{args.visualization_dataset_type}_{args.test_data_mode})/summary_{args.server_model}.txt'
        ...
        self.set_clients(clientTSPv4_ablation_2)
        ...
        if args.server_model == 'clip':
            ...
            self.global_model = TextEncoder_server_bert(self.args.classes, self.args.len_prompt,
                                                        pretrained_model_name=args.bert_model_path, CSC=self.args.CSC,
    ...
    def train(self):
        for i in tqdm(range(self.global_rounds + 1)):
            ...
            if self.global_text_protos is None:
                ...
                self.global_text_protos = self.args.EMA_alpha * self.global_text_protos + (
                            1 - self.args.EMA_alpha) * new_global_text_protos
                    ...
                    for j in range(self.args.prompt_epoch):
                        if self.args.TGP_loss != 0:
                            ...
                            for proto, y in proto_loader:
                                ...
                                loss = F.cross_entropy(-dist, y) * self.args.TGP_loss
