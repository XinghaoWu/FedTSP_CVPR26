...
class TextEncoder_server(nn.Module):
    ...
    def forward(self, global_vision_prototype):
        ...
        if isinstance(global_vision_prototype, defaultdict):
            # 允许全局仅包含部分类别：仅对存在原型的类别进行堆叠
            present_class_ids = sorted(global_vision_prototype.keys())
            if len(present_class_ids) == 0:
                raise ValueError("global_vision_prototype is empty; cannot compute logits.")
            feature_list = [global_vision_prototype[class_id] for class_id in present_class_ids]
        ...
        if isinstance(global_vision_prototype, defaultdict):
            text_features = text_features[present_class_ids]
