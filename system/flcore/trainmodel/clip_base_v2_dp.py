...
class PromptLearner_client(nn.Module):
    ...
    def __init__(
        ...
        # 初始化高斯噪声参数
        self.noise_std = noise_std  # 噪声标准差，可以根据需要调整
        ...
        if self.noise_enabled:
            # 获取tokenized_prompts的形状来生成对应大小的噪声
            seq_len = tokenized_prompts.shape[1]
            noise = torch.randn(self.batch_total, seq_len, ctx_dim, dtype=self.dtype) * self.noise_std
            self.register_buffer("fixed_noise", noise)
            print(f'fixed_noise: {self.fixed_noise}')
        else:
            self.register_buffer("fixed_noise", torch.zeros(1, 1, ctx_dim, dtype=self.dtype))
    ...
    def forward(self):
        ...
        with torch.no_grad():
            ...
            # temp_base_emb = base_emb.clone()
            # # 计算每个batch样本的L2 norm
            # # 将(batch_total, seq_len, dim)展平为(batch_total, seq_len * dim)
            # batch_size, seq_len, dim = temp_base_emb.shape
            # base_emb_flat = temp_base_emb.view(batch_size, -1)  # (batch_total, seq_len * dim)
            # l2_norms = torch.norm(base_emb_flat, p=2, dim=1)  # (batch_total,)
            # print(f"L2 norms for each batch sample: {l2_norms}")
            # exit()

        # 2) 克隆 base_emb 以避免 in-place 修改
        prompts_with_ctx = base_emb.clone()
        
        print(f'prompts_with_ctx: {prompts_with_ctx}')
        # 3) 添加固定的高斯噪声
        if self.noise_enabled:
            prompts_with_ctx = prompts_with_ctx + self.fixed_noise.to(self.device)
        print(f'prompts_with_ctx after noise: {prompts_with_ctx}')
        
        # 4) 替换正样本的前 n_ctx tokens
