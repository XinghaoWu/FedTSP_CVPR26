...
# 超参数列表
# lamda_list=(0 0.01 0.05 0.1 0.5 1 5 10 100)
...
python main.py --dataset=${Dataset} --num_classes=${num_classes} --model_family=${model_family} --local_learning_rate=0.01 --global_rounds=${global_rounds} --algorithm=FedProtoCos --local_epochs=5 --batch_size=${batch_size} --num_clients=${num_clients} --lamda=${lamda} --seed=0 --device_id=${device_id} --save_model=1 --tag=detach &
pids+=($!)
task_idx=$((task_idx + 1))
