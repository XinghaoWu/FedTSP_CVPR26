...
model_familys=("resnet4" "resnet6" "resnet8" "resnet10" "resnet18" "resnet34" "resnet50" "resnet101" "resnet152")
...
python main.py --dataset=${Dataset} --num_classes=${num_classes} --model_family=${model_family} --local_learning_rate=0.01 --global_rounds=${global_rounds} --algorithm=FedProtoCKA --local_epochs=5 --batch_size=${batch_size} --num_clients=${num_clients} --lamda=${lamda} --seed=0 --device_id=${device_id} --save_model=1 &
pids+=($!)
task_idx=$((task_idx + 1))
