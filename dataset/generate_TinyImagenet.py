...
class ImageFolder_custom(DatasetFolder):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None):
        ...
        print(imagefolder_obj.class_to_idx)
...
def generate_dataset(niid, balance, partition, args):
    ...
    # record the class names in the dataset
    csv_path = "rawdata/TinyImagenet//tiny-imagenet-200/tinyimagenet_classes_sorted_lex.csv"  # 替换为实际路径
    df = pd.read_csv(csv_path)

    # Extract class labels into a list
    classes = df['Class Label'].tolist()
    ...
    save_file(config_path, train_path, test_path, train_data, test_data, num_clients, num_classes,
              statistic, niid, balance, partition, args.alpha, classes)
